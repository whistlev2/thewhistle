...
exports.getFormSlug = async function (reportID) {
    try {
        ...
        const slugs = await db.query(`SELECT slug FROM subforms JOIN rawresponse ON rawresponse.form_id=subforms.typeform_id WHERE rawresponse.id=${parseInt(reportID)}`);
